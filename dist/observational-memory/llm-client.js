...
export class LLMClient {
     ...
     */
    async extractObservations(messages, sourceFile) {
        ...
        response = await GracefulDegradation.retry(`LLM extraction (${this.config.llmProvider})`, async () => {
     ...
     */
    parseObservations(response, sourceFile) {
        const observations = [];
    ...
    async callGemini(messages) {
        ...
        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${this.config.llmModel}:generateContent?key=${this.config.apiKey}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
